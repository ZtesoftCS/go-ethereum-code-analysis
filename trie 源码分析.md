...
## Patricia Tries (前缀树)
前缀树根Trie树的不同之处在于Trie树给每一个字符串分配一个节点，这样如果很多很长的，但是又没有公共节点的字符串就会导致Trie树退化成一个数组，在以太坊里面可以会又黑客构造很多这种节点造成拒绝服务攻击。前缀树的不同之处在于如果节点公共前缀，那么就使用公共前缀，否则就把剩下的所有节点插入同一个节点。
...
- 如果不是上面两种情况，那么使用分支节点进行集合切分，因为key是使用HP进行编码的，所以可能的分支只有0-15这16个分支。 如果有节点刚好在这里完结了， 那么第17个节点就是为这种情况准备的。
